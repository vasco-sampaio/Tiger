...
namespace astclone
{
  ...
  void Cloner::operator()(const ast::IfExp& e)
  {
    ...
    Exp* then = recurse(e.then_clause_get());
    if (e.else_clause_get() == nullptr)
      result_ =  new IfExp(location, test, then, nullptr);
    else
    {
      Exp* else_c = recurse(*e.else_clause_get());
      result_ =  new IfExp(location, test, then, else_c);
    }
  }
  ...
  void Cloner::operator()(const ast::StringExp& e)
  {
    // DONE: Some code was deleted here.
    const Location& location = e.location_get();
    std::string string_ = e.string_get();
    result_ = new StringExp(location, string_);
  }
